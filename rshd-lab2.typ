...
*Цель работы* — настроить процедуру периодического резервного копирования базы данных, сконфигурированной в ходе лабораторной работы №1, а также разработать и отладить сценарии восстановления в случае сбоев.

*Узлы:*
- *pg116* (основной) — `postgres1@pg116`, `PGDATA=$HOME/aoj42`, порт 9748, БД `bestbluemath`, табличные пространства `ts_ydp10` и `ts_zcc31`
- *pg117* (резервный) — `postgres4@pg117`, FreeBSD 14.1, PostgreSQL 16.4

*Этапы:*

+ *Резервное копирование* — холодный rsync-бэкап по cron раз в сутки, хранить 14 копий, подсчитать объём за 30 дней
+ *Потеря основного узла* — восстановить СУБД на резервном узле из последней копии
+ *Повреждение файлов БД* — удалить конфигурационные файлы, восстановить из копии с переносом табличных пространств
+ *Логическое повреждение данных* — восстановление через WAL PITR (point-in-time recovery)
